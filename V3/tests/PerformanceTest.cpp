...
public:
    ...
    static void warmup() 
    {
        ...
        // 释放预热内存
        for (const auto& item : warmupPtrs) 
        {
            MemoryPool::deallocate(item.first, item.second);  // 使用实际分配的大小进行释放
        }
    ...
    static void testSmallAllocation() 
    {
        ...
        // 测试new/delete
        {
            ...
            for (auto& ptrs : sizePtrs) 
            {
                for (const auto& item : ptrs) 
                {
                    delete[] static_cast<char*>(item.first);
                }
            }
        ...
        auto threadFunc = [ALLOCS_PER_THREAD](bool useMemPool) 
        {
            ...
            for (size_t i = 0; i < ALLOCS_PER_THREAD; ++i) 
            {
                ...
                if (i % 1000 == 0)  // 每1000次分配
                {
                    ...
                    for (int j = 0; j < 50; ++j) 
                    {
                        size_t pressureSize = SIZES[rand() % NUM_SIZES];
                        void* pressurePtr = useMemPool ? MemoryPool::allocate(pressureSize)
                                                       : new char[pressureSize];
                        pressurePtrs.push_back({pressurePtr, pressureSize});
                    }
                    
                    // 立即释放这些内存，测试内存池的回收效率
                    for (const auto& item : pressurePtrs) 
                    {
                        if (useMemPool) 
                        {
                            MemoryPool::deallocate(item.first, item.second);
